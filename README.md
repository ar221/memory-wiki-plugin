...
A compiled-wiki memory plugin for [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent). Extracted from a local fork at `~/.hermes/hermes-agent/` (7-phase implementation). 55 tests, all passing.
...
memory-wiki-plugin/
├── README.md                                  # This file
├── SCHEMA.md                                  # Canonical page-authoring reference
...
└── tests/
    ├── conftest.py             # Hermetic test fixtures (from hermes-agent)
    ├── scripts/
...
All 55 tests use the `_hermetic_environment` fixture from `tests/conftest.py`, which redirects `HERMES_HOME` to a per-test tempdir so no test ever touches the real `~/.hermes`.

---

## 3. Install

### Step A — copy files into your hermes-agent clone
...
The script validates that the target is a git repository, then copies every plugin file to its canonical path under the hermes-agent tree. It prints each copied path.

### Step B — two manual patches to hermes-agent source
...
Add the following near the other `cmd_*` functions (e.g. after `cmd_cron`):
...
mw_parser = subparsers.add_parser(
    ...
    description="Compiled markdown memory wiki for Hermes",
)
...
mw_ingest = mw_subparsers.add_parser(
    "ingest-artifact", help="Copy an artifact into raw/artifacts/ and log it"
)
...
mw_promote = mw_subparsers.add_parser(
    "promote", help="Promote candidates from _meta/promotion-reports/ into compiled pages"
)
mw_promote.add_argument(
    "--since", default="24h", help="Time window: '24h', '7d', or 'today' (default: 24h)"
)
mw_promote.add_argument(
    "--auto-approve", action="store_true", dest="auto_approve",
    help="Skip the confirmation gate (use with caution)"
)
...
After both patches, the `hermes memory-wiki` command is available with 6 subcommands.
...
Print the wiki root path. Respects the `HERMES_HOME` environment variable and the profile-aware logic in `hermes_constants.get_hermes_home()`.
...
|---|---|---|
| `missing_frontmatter` | error | Page does not start with a `---` YAML block |
| `missing_required_field` | error | One of `title`, `created`, `updated`, `type`, `status` is absent or blank |
| `broken_wikilink` | error | `[[target]]` reference has no matching `.md` file on disk |
| `missing_from_index` | error | Page has no `[[rel/path]]` entry in `index.md` |
| `page_too_long` | warning | Page exceeds 200 lines |
| `unknown_tag` | warning | Tag not in the canonical list in `SCHEMA.md` |
...
Collision avoidance: if the destination filename already exists, a `-2`, `-3`, ... suffix is appended.

### `hermes memory-wiki search <query> [--limit N]`

Keyword search across all compiled pages. Scores by term frequency; titles and `index.md` one-liner descriptions receive a 2x bonus weight. Returns up to `--limit` results (default 5), sorted by score.
...
Review promotion candidates from `_meta/promotion-reports/` and generate compiled page stubs. Candidates are written there by `WikiMemoryProvider.sync_turn` heuristics during live sessions.
...
The `--since` flag accepts: `Nh` (N hours ago), `Nd` (N days ago), or `today` (since midnight UTC). When more than 3 topic groups are proposed, the CLI asks for confirmation unless `--auto-approve` is set.

---

## 5. Schema enforcement

All compiled pages must follow `SCHEMA.md` at the repo root. The `lint` command enforces it. See `SCHEMA.md` for the full canonical reference.
...
- **Compiled pages live under `entities/`, `concepts/`, `decisions/`, `incidents/`, `queries/`** only.
- **Every compiled page needs YAML frontmatter** with `title`, `created`, `updated`, `type`, `scope`, `status`, `tags`, and `sources`.
- **Every compiled page must appear in `index.md`** — referenced as `[[rel/path/without/ext]]` or `[[rel/path|Label]]`. Both forms are recognised by the linter.
- **Every operation appends to `log.md`** — the log is append-only and chronological.
...
## 6. Security constraints on ingest

The `memory_wiki_ingest_artifact` tool (and the `ingest-artifact` CLI subcommand) enforce two layers of security.

### Source allowlist

Files must be under one of these roots (resolved to absolute paths):
...
Files from any other location are rejected with an error.

### Secret detection

Two complementary checks:

1. **Filename heuristic**: filenames containing `auth`, `secret`, `token`, `password`, `credential`, or `.env` are rejected.
2. **Content scan**: the first 4KB of the file is scanned for common credential patterns:
   - GitHub PATs: `ghp_[A-Za-z0-9]{36}`
   - OpenAI/similar API keys: `sk-[A-Za-z0-9]{32,}`
   - Bearer tokens: `Bearer [A-Za-z0-9._-]{20,}`
   - PEM headers: `-----BEGIN [A-Z ]+-----`
   - Inline passwords: `password: value` or `password = value`

   Files over 50KB skip the content scan (name-only check applies).

---

## 7. MemoryProvider plugin

`plugins/memory/wiki/__init__.py` provides a `WikiMemoryProvider` that Hermes loads as a memory plugin.

To register it, call `register(ctx)` from your plugin loader, or follow the hermes-agent plugin registration pattern for memory providers.

`WikiMemoryProvider` implements 5 lifecycle hooks:

### `initialize(session_id, **kwargs)`

Resolves the wiki root from `kwargs["hermes_home"]` or `hermes_constants.get_hermes_home()`. Records whether this is a primary session or a subagent context via `kwargs["agent_context"]`. Subagent contexts skip `sync_turn` writes to avoid promotion noise from parallel workers.

### `system_prompt_block()`

Returns a static one-paragraph notice injected into the system prompt when the wiki's `index.md` exists. Directs the LLM to prefer compiled wiki pages over raw transcript recall and names the four available tools.

### `prefetch(query, session_id="")`

Called with the first user message. Runs a keyword search (`limit=3`) and returns a brief markdown block of the top results, prepended to the conversation context. Returns an empty string if the wiki does not exist or search finds nothing.

### `sync_turn(user_content, assistant_content, session_id="")`

Called after every turn in primary sessions. Detects two classes of durable signal:
...
When a signal fires, writes a candidate file to `_meta/promotion-reports/` with the trigger phrase, a 500-character user excerpt, and a 500-character assistant excerpt.

### `on_session_end(messages)`

If the session has 5 or more messages and is a primary context, writes a session summary candidate with the first and last user messages.
...
The promotion pipeline converts ephemeral session signals into durable wiki pages. There is no LLM in the promotion loop — it is pure text manipulation. The LLM author fills in content after stubs are created.

Full flow:

1. During live sessions, `WikiMemoryProvider.sync_turn` detects signals and writes candidate files to `~/.hermes/memory-wiki/_meta/promotion-reports/`. At session end, `on_session_end` may write an additional summary candidate.

2. Run `hermes memory-wiki promote --since 24h` to collect and review candidates:
   - `parse_since` converts the `--since` string to a UTC cutoff datetime.
   - `load_candidates` reads all `.md` files whose `Date:` header is on or after the cutoff. Files whose names end with `-promotion.md` (output reports from prior runs) are skipped.
   - `classify_candidates` groups candidates by topic via keyword heuristics: `"decided"/"decision"` -> decision, `"broke"/"bug"/"error"` -> incident, `"workflow"/"process"/"how to"` -> workflow, `"project"/"campaign"/"task"` -> project, everything else -> preference.
   - `propose_updates` builds a proposal per topic group. For each group it runs a wiki search with the first candidate as the query; if the top result scores >= 3 and lives in the topic directory, it proposes an **update** (append to existing page). Otherwise it proposes a **create** (new stub page with frontmatter).
   - If more than 3 topic groups are proposed, the CLI asks for confirmation. Pass `--auto-approve` to bypass.
   - `apply_proposals` writes the changes. New pages are added to `index.md`. All changes are logged to `log.md`.
   - `write_promotion_report` writes a summary to `_meta/promotion-reports/YYYY-MM-DD-promotion.md`.

3. Open the newly created stub pages and fill in the content. Stubs have `status: stub` — change to `status: active` when complete.
...
├── index.md                           # master page index (every compiled page listed here)
...
`index.md` is the single source of truth for what compiled pages exist. Every compiled page must have a `[[rel/path]]` entry in `index.md`. The linter enforces this.

`log.md` is append-only. Every operation (init, ingest, lint, promote, query) appends a dated header line. Never truncate or reorganize it.

---

## 10. Running tests

Tests require a hermes-agent environment with `hermes_constants` importable and `tests/conftest.py` present.

```bash
cd /path/to/hermes-agent   # tests must run inside hermes-agent to resolve imports
...
To run only the memory-wiki tests:

```bash
pytest tests/ -k "memory_wiki or wiki_provider" -v
```
...
|---|---|---|
| `test_memory_wiki_lint.py` | 12 | Clean wiki, missing frontmatter, missing required fields, broken wikilinks, missing from index, piped wikilink recognition, page-too-long warning, report file, same-day no-overwrite, ingest, ingest collision |
| `test_memory_wiki_promote.py` | 13 | `parse_since` (hours/days/today), date filter, classify (decision/workflow/preference default), `apply_proposals` (create + update), no candidates, skip output files, `propose_updates` required keys, report collision |
| `test_memory_wiki_cli.py` | 8 | `path`, `init`, `lint` (exit 0 + exit 1), `ingest-artifact`, `search`, no-subcommand usage |
...
| `test_memory_wiki_tool.py` | 11 | `search` (results + no wiki), `read` (existing + nonexistent + traversal + no wiki), `ingest_artifact` (success + secret name rejected + credential content rejected + disallowed root), `lint` (clean + no wiki) |
